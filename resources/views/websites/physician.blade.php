...
<body>
    <div id="app">
        <physician-website-component userSubscribed="{{$userSubscribed ?? ''}}" user_id="{{$user_id}}" email="{{$email}}" can="{{$can}}" social="{{$socials ?? null}}" tenant='{{$tenantID ?? null}}' preview='{{$preview ?? 0}}' title='{{$title ?? null}}' template_id="{{$template_id}}" template="{{$template}}" user="{{$user ?? ''}}"></physician-website-component>
    </div>
